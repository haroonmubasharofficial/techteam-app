...
class PurchaseController extends Controller {
    ...
    private function nextNumber(string $date): string { $prefix='PUR-'.date('Y',strtotime($date)).'-';$last=Purchase::where('purchase_number','like',$prefix.'%')->lockForUpdate()->orderByDesc('id')->value('purchase_number');$next=$last?((int)substr($last,-5))+1:1;return $prefix.str_pad((string)$next,5,'0',STR_PAD_LEFT); }
}
