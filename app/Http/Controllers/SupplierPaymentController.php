...
class SupplierPaymentController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse {
        ...
        $payment=DB::transaction(function()use($data){$payment=new SupplierPayment();$payment->payment_number=app(DocumentNumberService::class)->next('supplier_payment',$data['payment_date']);$payment->fill(['supplier_id'=>$data['supplier_id'],'payment_date'=>$data['payment_date'],'payment_method'=>$data['payment_method'],'reference'=>$data['reference']??null,'amount'=>$data['amount'],'status'=>'posted','notes'=>$data['notes']??null]);$payment->save();$allocated=0.0;foreach($data['allocations']??[] as $allocation){$purchase=Purchase::where('id',$allocation['purchase_id'])->where('supplier_id',$payment->supplier_id)->lockForUpdate()->firstOrFail();$alreadyPaid=(float)$purchase->paymentAllocations()->sum('amount');$outstanding=max(0,(float)$purchase->total_amount-$alreadyPaid);abort_if((float)$allocation['amount']>$outstanding+0.0001,422,"Allocation exceeds outstanding balance for {$purchase->purchase_number}.");$allocated+=(float)$allocation['amount'];abort_if($allocated>(float)$payment->amount+0.0001,422,'Allocated amount cannot exceed the payment amount.');$payment->allocations()->create(['purchase_id'=>$purchase->id,'amount'=>$allocation['amount']]);$purchase->update(['status'=>$outstanding-(float)$allocation['amount']<=0.0001?'paid':'partially_paid']);}DB::table('party_ledger_entries')->insert(['customer_id'=>null,'supplier_id'=>$payment->supplier_id,'entry_date'=>$payment->payment_date,'entry_type'=>'payment','reference_type'=>'supplier_payment','reference_id'=>$payment->id,'debit'=>$payment->amount,'credit'=>0,'description'=>'Supplier payment '.$payment->payment_number,'created_at'=>now(),'updated_at'=>now()]);return $payment;});
