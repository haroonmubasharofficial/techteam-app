...
class CustomerPaymentController extends Controller
{
    ...
    private function nextReceiptNumber(string $date): string {$year=date('Y',strtotime($date));$prefix="RCPT-{$year}-";$last=CustomerPayment::where('receipt_number','like',$prefix.'%')->lockForUpdate()->orderByDesc('id')->value('receipt_number');$next=$last?((int)substr($last,-5))+1:1;return $prefix.str_pad((string)$next,5,'0',STR_PAD_LEFT);}
}
