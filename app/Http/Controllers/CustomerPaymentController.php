...
class CustomerPaymentController extends Controller
{
    ...
    public function invoices(Customer $customer): \Illuminate\Http\JsonResponse { $invoices=Invoice::where('customer_id',$customer->id)->whereIn('status',['draft','delivered','partially_delivered','partially_paid','posted'])->orderBy('invoice_date')->orderBy('id')->get()->map(function($invoice){$allocated=(float)$invoice->paymentAllocations()->sum('amount');return ['id'=>$invoice->id,'invoice_number'=>$invoice->invoice_number,'invoice_date'=>$invoice->invoice_date?->format('Y-m-d'),'total'=>(float)$invoice->total_amount,'paid'=>$allocated,'outstanding'=>max(0,(float)$invoice->total_amount-$allocated)];})->filter(fn($i)=>$i['outstanding']>0)->values();return response()->json($invoices); }
