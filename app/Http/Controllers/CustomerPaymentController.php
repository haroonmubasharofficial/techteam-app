...
class CustomerPaymentController extends Controller
{
    ...
    public function index(Request $request): View { $payments=CustomerPayment::with('customer')->when($request->filled('q'),function($query)use($request){$term=trim($request->string('q')->toString());$query->where(function($q)use($term){$q->where('receipt_number','like',"%{$term}%")->orWhereHas('customer',fn($cq)=>$cq->where('company_name','like',"%{$term}%"));});})->latest('payment_date')->latest('id')->paginate(20)->withQueryString();return view('customer_payments.index',compact('payments')); }
    ...
    public function invoices(Customer $customer): \Illuminate\Http\JsonResponse { $invoices=Invoice::where('customer_id',$customer->id)->where('status','!=','draft')->whereIn('status',['issued','delivered','partially_delivered','partially_paid','posted'])->orderBy('invoice_date')->orderBy('id')->get()->map(function($invoice){$allocated=(float)$invoice->paymentAllocations()->sum('amount');return ['id'=>$invoice->id,'invoice_number'=>$invoice->invoice_number,'invoice_date'=>$invoice->invoice_date?->format('Y-m-d'),'total'=>(float)$invoice->total_amount,'paid'=>$allocated,'outstanding'=>max(0,(float)$invoice->total_amount-$allocated)];})->filter(fn($i)=>$i['outstanding']>0)->values();return response()->json($invoices); }
    ...
    public function receivables(): View {$customers=Customer::where('is_active',true)->orderBy('company_name')->get()->map(function($customer){$invoices=Invoice::where('customer_id',$customer->id)->where('status','!=','draft')->get(['id','total_amount']);$invoiced=(float)$invoices->sum('total_amount');$paid=(float)DB::table('customer_payment_allocations')->whereIn('invoice_id',$invoices->pluck('id'))->sum('amount');return ['customer'=>$customer,'invoiced'=>$invoiced,'paid'=>$paid,'receivable'=>max(0,$invoiced-$paid)];})->filter(fn($row)=>$row['receivable']>0)->values();return view('customer_payments.receivables',compact('customers'));}
