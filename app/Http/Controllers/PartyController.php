...
class PartyController extends Controller
{
    ...
    public function storeCustomer(Request $request): RedirectResponse { $data=$this->customerData($request); $data['is_active']=true; Customer::create($data); return redirect()->route('parties.customers')->with('success','Customer created.'); }
    public function editCustomer(Customer $customer): View { return view('parties.customer_edit',compact('customer')); }
    public function updateCustomer(Request $request, Customer $customer): RedirectResponse { $data=$this->customerData($request); $data['is_active']=$request->boolean('is_active'); $customer->update($data); return redirect()->route('parties.customers')->with('success','Customer updated.'); }
    ...
    public function storeSupplier(Request $request): RedirectResponse { $data=$this->supplierData($request); $data['is_active']=true; Supplier::create($data); return redirect()->route('parties.suppliers')->with('success','Supplier created.'); }
    public function editSupplier(Supplier $supplier): View { return view('parties.supplier_edit',compact('supplier')); }
    public function updateSupplier(Request $request, Supplier $supplier): RedirectResponse { $data=$this->supplierData($request); $data['is_active']=$request->boolean('is_active'); $supplier->update($data); return redirect()->route('parties.suppliers')->with('success','Supplier updated.'); }
