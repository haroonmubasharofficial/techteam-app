...
class ReportController extends Controller
{
    public function index(Request $request): View
    {
        ...
        $sales = Invoice::where('status', '!=', 'draft')->whereBetween('invoice_date', [$from, $to])
            ->selectRaw('COALESCE(SUM(total_amount),0) total')
            ->selectRaw('COALESCE(SUM(actual_cost_total),0) cost')
            ->selectRaw('COALESCE(SUM(actual_profit),0) profit')->first();
        ...
        $receivable = Invoice::where('status', '!=', 'draft')->sum('total_amount');
        ...
        $payable = Purchase::sum('total_amount');
        ...
        $stockValue = 0.0;
        if ($warehouseId) {
            $rows = DB::table('stock_transactions')->where('warehouse_id',$warehouseId)->select('product_id')
                ->selectRaw('SUM(quantity_in-quantity_out) qty')->selectRaw('SUM(quantity_in*unit_cost) inbound_cost')
                ->selectRaw('SUM(quantity_in) inbound_qty')->groupBy('product_id')->get();
            foreach ($rows as $row) {
                $qty=(float)$row->qty; $avg=(float)$row->inbound_qty>0?(float)$row->inbound_cost/(float)$row->inbound_qty:0;
                $stockValue += max(0,$qty)*$avg;
            }
        }
